...
# 3 affect
# positive
plot(trump2$hopeful, main = "Hopeful")
```

```{r,echo = FALSE}
plot(trump2$proud, main = "Proud")
```
...
```{r,echo = FALSE}
# negative
plot(trump2$angry, main = "Angry")
```

```{r,echo = FALSE}
plot(trump2$afraid, main = "Afraid")
```

```{r,echo = FALSE}
plot(trump2$disgusted, main = "Disgusted")
```
...
As part of Trump's promise in 2016, building wall in border the Mexico was one of the biggest issue people were concerned or interested in. Similar to results from the negative affection keywords, respondents are polarized when being asked about how they view this promise. People either strongly favor it or strongly oppose this it, so we are not able to conlcude from this histogram whether this promise has helped Trump win.
...
hist(parties$like_dislike,xlim = c(-1,10), main = "Favor for Republicans", xlab = "Rating") # favor R
...
hist(dislike$like_dislikeD, breaks = 10, main = "Favor for Democrats", xlab = "Rating")
```
...
par(mfrow = c(1,2))
hist(prefs$preference, breaks = 2,xaxt="n", main = "Voted for Hillary Clinton", xlab = "Preference")
axis(1, at=seq(10, 11, by=1), labels=c("not strong preference", "strong preference"))

hist(prefs2$preference, breaks = 2,xaxt="n", main = "Voted for Donald Trump", xlab = "Preference")
